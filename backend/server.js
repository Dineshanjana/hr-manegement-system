...
app.use(session({
  secret: '626d50aeade1d2ea701e706fb707836ac6cd30ea939807c22d6640fe5d5ca67866ec062a5cb7',
  resave: false,
...
app.put('/postings/:personnel_id', (req, res) => {
  ...
  const values = [posted_to, start_date, end_date, no_of_days, prefix_date, suffix_date, remarks, reported_back, reporting_date, personnel_id];
...
app.get('/api/leave', (req, res) => {
  const sql = 'SELECT l.leave_id, p.first_name, p.last_name, l.service_number, l.start_date, l.end_date, l.status, l.reporting_date FROM \`leave\` l LEFT JOIN personnel p ON l.personnel_id = p.personnel_id';
...
app.get('/api/leave/:leaveId', (req, res) => {
  ...
  const sql = 'SELECT l.leave_id, p.first_name, p.last_name, l.service_number, l.start_date, l.end_date, l.status, l.reporting_date FROM \`leave\` l LEFT JOIN personnel p ON l.personnel_id = p.personnel_id WHERE l.leave_id = ?';
...
app.post('/api/leave', (req, res) => {
  const { personnel_id, service_number, start_date, end_date, status, reporting_date } = req.body;
  const sql = 'INSERT INTO \`leave\` (personnel_id, service_number, start_date, end_date, status, reporting_date) VALUES (?, ?, ?, ?, ?, ?)';

  db.query(sql, [personnel_id, service_number, start_date, end_date, status, reporting_date], (err, result) => {
    if (err) {
...
app.put('/api/leave/:leaveId', (req, res) => {
  const leaveId = req.params.leaveId;
  const { personnel_id, service_number, start_date, end_date, status, reporting_date } = req.body;
  const sql = 'UPDATE \`leave\` SET personnel_id = ?, service_number = ?, start_date = ?, end_date = ?, status = ?, reporting_date = ? WHERE leave_id = ?';

  db.query(sql, [personnel_id, service_number, start_date, end_date, status, reporting_date, leaveId], (err, result) => {
    if (err) {
